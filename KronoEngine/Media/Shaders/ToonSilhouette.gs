...
void EmitEdgeQuad(vec3 e0, vec3 e1) {
	...
	gl_Position = vec4(e0.xy, e0.z, 1.0);
	EmitVertex();
	gl_Position = vec4(e0.xy - n, e0.z, 1.0);
	EmitVertex();
	gl_Position = vec4(e1.xy, e1.z, 1.0);
	EmitVertex();
	gl_Position = vec4(e1.xy - n, e1.z, 1.0);
	EmitVertex();
...
void main() {
	...
	if (gl_in[0].gl_Position.w > 0 && gl_in[1].gl_Position.w > 0 && gl_in[2].gl_Position.w > 0 && gl_in[3].gl_Position.w > 0 && gl_in[4].gl_Position.w > 0 && gl_in[5].gl_Position.w > 0) {
